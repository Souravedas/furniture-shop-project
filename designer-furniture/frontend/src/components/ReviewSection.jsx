  ...
  const fetchReviews = async () => {
    try {
      ...
      res.data.forEach((review) => {
        ...
        if (
          !latestReviewPerUser[key] ||
          new Date(review.createdAt) >
            new Date(latestReviewPerUser[key].createdAt)
        ) {
      ...
      <div className="review-carousel">
        {allReviews.length === 0 ? (
          ...
          >
            ...
            <div
              className={`review-card ${fade ? "fade" : ""}`}
              key={allReviews[currentReview]?.createdAt}
            >
              <div className="avatar">
                <div className="initials">
                  {allReviews[currentReview]?.profilePicture ? (
                    <img
                      src={allReviews[currentReview].profilePicture}
                      alt="Profile"
                      className="review-pic"
                      onError={(e) => (e.target.src = "/default-avatar.png")}
                    />
                  ) : (
                    <div className="get-initials">
                      {getInitials(allReviews[currentReview]?.name)}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="stars">
                {[...Array(5)].map((_, index) => (
                  <i
                    key={index}
                    className={`fa-star ${
                      index < allReviews[currentReview]?.rating
                        ? "fas filled"
                        : "far"
                    }`}
                  />
              ...
              <small>
                {new Date(
                  allReviews[currentReview]?.createdAt
                ).toLocaleDateString()}
              </small>
            </div>
