...
export const getAllReviews = async (req, res) => {
  try {
    const users = await User.find({ "reviews.0": { $exists: true } }).select(
      "name profilePicture reviews"
    );
...
export const deleteReview = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      "reviews.createdAt": new Date(createdAt),
    });
    ...
    user.reviews = user.reviews.filter(
      (review) =>
        review.createdAt.toISOString() !== new Date(createdAt).toISOString()
    );
